...
class BackupManager {
  ...
  generateBackupWrapper(profile) {
    const databases = profile.Databases && profile.Databases.length > 0 ? profile.Databases : ['--all-databases'];
    const dbList = databases.map(d => d === '--all-databases' ? '--all-databases' : `\"${d}\"`).join(' ');
    const args = [
      `\"${this.mysqldumpPath}\"`,
      `--host=\"${profile.Host}\"`,
      `--port=${profile.Port}`,
    ...
    const parts = (profile.CronExpression || '0 2 * * *').split(/\s+/);
    ...
    return `# Auto-generated by Kyrion Kronou - Backup Service Wrapper
# Profile: ${profile.Name}
# Cron: ${profile.CronExpression}

$backupPath = \"${profile.BackupPath}\";
$namingPattern = \"${profile.NamingPattern}\";
$compress = \"${profile.Compression}\";
$compressLevel = ${profile.CompressionLevel || 5};

if (!(Test-Path $backupPath)) { New-Item -ItemType Directory -Path $backupPath -Force | Out-Null }

function Get-NextRun {
  $now = Get-Date
  $next = [datetime]::Today.AddHours(${hour}).AddMinutes(${minute})
...
while ($true) {
  ...
  try {
    $databases = ${databases.length > 0 ? `@(${databases.map(d => `\"${d}\"`).join(', ')})` : `@('\"--all-databases\"')`};
    foreach ($db in $databases) {
      $now = Get-Date
      $dateStr = $now.ToString('yyyyMMdd')
      $timeStr = $now.ToString('HHmmss')
      $filename = $namingPattern.Replace('{database}', $db.Replace('--all-databases','all_databases')).Replace('{date}', $dateStr).Replace('{time}', $timeStr)
      $dumpPath = Join-Path $backupPath ($filename + '.sql')
      $finalPath = Join-Path $backupPath ($filename + $(if ($compress -eq 'zip') { '.sql.zip' } elseif ($compress -eq '7z') { '.sql.7z' } else { '.sql' }))

      $cmdArgs = \"--host=\"${profile.Host}\" --port=${profile.Port} --user=\"${profile.User}\" --password=\"${profile.Password}\" ${profile.ExtraArgs || ''} $db --result-file=\"$dumpPath\"\"
      $cmd = \"\"${this.mysqldumpPath}\" $cmdArgs\"
      Invoke-Expression $cmd

      if ($compress -ne 'none' -and (Test-Path $dumpPath)) {
        if ($compress -eq 'zip') {
          Compress-Archive -Path $dumpPath -DestinationPath $finalPath -CompressionLevel Optimal
        } elseif ($compress -eq '7z') {
          & \"C:\\Program Files\\7-Zip\\7z.exe\" a -t7z -mx=$compressLevel $finalPath $dumpPath
        }
        Remove-Item $dumpPath -Force -ErrorAction SilentlyContinue
      }
    }
  } catch {
    Write-EventLog -LogName Application -Source 'KyrionBackup' -EventId 1 -EntryType Error -Message $_.Exception.Message -ErrorAction SilentlyContinue
  }
}
`;
  }
